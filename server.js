...
app.get('/api/stock', adminOnly, async (req, res) => {
  if (!STOCK_SHEET_URL) return res.status(503).json({ error: 'STOCK_SHEET_URL not set' });
  try {
    const d = await sheetFetch(`${STOCK_SHEET_URL}?action=getAll&t=${Date.now()}`);
    res.json(d);
...
app.get('/api/stock/write', adminOnly, async (req, res) => {
  ...
  try {
    ...
    const d = await sheetFetch(`${STOCK_SHEET_URL}?${params}`);
    res.json(d);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/shop/products', async (req, res) => {
  if (!SHOP_SHEET_URL) return res.status(503).json({ error: 'SHOP_SHEET_URL not set' });
  try {
    const d = await sheetFetch(`${SHOP_SHEET_URL}?action=getAll&t=${Date.now()}`);
    const products = (d.inventory || [])
      .filter(p => (parseInt(p.qty) || 0) > 0)
      .map(p => ({
        id: p.id, name: p.name, brand: p.brand || '',
        variant: p.variant || '', size: p.size || '',
        country: p.country || '', image: p.image || p.img || '',
...
app.get('/api/shop/order', async (req, res) => {
  ...
  try {
    const params = new URLSearchParams(req.query);
    const d = await sheetFetch(`${SHOP_SHEET_URL}?${params}`);
    res.json(d);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/shop/track', async (req, res) => {
  ...
  try {
    ...
    res.json({
      id: order.id, product: order.product, customer: order.customer,
      status: order.status, date: order.date, total: order.total,
      parcelid: order.parcelid || '', trackinglink: order.trackinglink || ''
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/inbox', adminOnly, async (req, res) => {
  if (!INBOX_SHEET_URL) return res.status(503).json({ error: 'INBOX_SHEET_URL not set' });
  try {
    const params = new URLSearchParams(req.query);
    const d = await sheetFetch(`${INBOX_SHEET_URL}?${params}`);
    res.json(d);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/inbox/reply', adminOnly, async (req, res) => {
  if (!N8N_REPLY_URL) return res.status(503).json({ error: 'N8N_REPLY_URL not set' });
  try {
