// ============================================================
// MINSAH BEAUTY — Google Apps Script v3 (Final)
// Sheet tabs: Inventory, Orders, Suppliers, Restock
//
// HOW TO USE:
// 1. Google Sheet → Extensions → Apps Script
// 2. পুরো code paste করো (আগেরটা delete করে)
// 3. Save (Ctrl+S)
// 4. Deploy → New deployment → Web App → Anyone → Deploy
// 5. URL copy করো → Dokploy STOCK_SHEET_URL তে paste করো
// ============================================================
...
// Inventory:
//   0=ID, 1=Name, 2=Brand, 3=Country, 4=Variant, 5=Size,
//   6=BuyPrice, 7=SellPrice, 8=Qty, 9=Supplier,
//   10=LastBuyDate, 11=LowestBuy, 12=Image,
//   13=TotalSold, 14=LastSoldDate
//
// Orders:
//   0=ID, 1=ParcelID, 2=TrackingLink, 3=Customer, 4=Phone,
//   5=Product, 6=ProductID, 7=Variant, 8=Qty, 9=Total,
//   10=Status, 11=Date, 12=Note, 13=Items(JSON),
//   14=Address, 15=District, 16=Thana
//
// Suppliers:
//   0=ID, 1=Name, 2=Phone, 3=Phone2, 4=Phone3, 5=Address
//
// Restock:
//   0=ID, 1=ProductID, 2=ProductName, 3=AddedQty,
//   4=PurchasePrice, 5=Supplier, 6=Date, 7=Note

// ── Entry points ──
function doGet(e)  { return handle(e); }
function doPost(e) { return handle(e); }

// ── Sheet auto-create helper ──
...
function handle(e) {
  ...
  const ordS = getOrCreate(ss, SHEETS.ORD, [
    ...
    'Status','Date','Note','Items','Address','District','Thana'
  ]);
  ...
  // GET params + POST body উভয়ই support
  ...
  if (action === 'updateProduct') {
    ...
    if (p.buydate   !== undefined) fields[10] = p.buydate;
  ...
  if (action === 'restock') {
    ...
    for (let i = 1; i < invData.length; i++) {
      if (String(invData[i][0]) === String(p.productId)) {
        ...
        invS.getRange(i+1, 7).setValue(price);                                           // BuyPrice
        invS.getRange(i+1, 9).setValue(newQty);                                          // Qty
        invS.getRange(i+1, 10).setValue(p.supplier || invData[i][9]);                   // Supplier
        invS.getRange(i+1, 11).setValue(p.date || new Date().toISOString().slice(0,10)); // LastBuyDate
        invS.getRange(i+1, 12).setValue(Math.min(curLowest, price));                     // LowestBuy
        break;
      }
    }
  ...
  if (action === 'addOrder') {
    // admin দেওয়া ID use করো, না হলে generate
    ...
    ordS.appendRow([
      id,
      p.parcelId     || '',          // 1  ParcelID
      p.trackingLink || '',          // 2  TrackingLink
      p.customer     || '',          // 3  Customer
      p.phone        || '',          // 4  Phone
      p.product      || '',          // 5  Product (summary)
      p.productId    || '',          // 6  ProductID
      p.variant      || '',          // 7  Variant
      parseInt(p.qty)     || 1,      // 8  Qty
      parseFloat(p.total) || 0,      // 9  Total
      p.status || 'pending',         // 10 Status
      p.date   || new Date().toISOString().slice(0, 10), // 11 Date
      p.note     || '',              // 12 Note
      itemsJson,                     // 13 Items (JSON)
      p.address  || '',              // 14 Address ✅
      p.district || '',              // 15 District ✅
      p.thana    || ''               // 16 Thana ✅
    ]);
    return out({ success: true, id });
  }
  ...
  if (action === 'updateOrder') {
    ...
    if (p.address      !== undefined) fields[14] = p.address;   // ✅
    if (p.district     !== undefined) fields[15] = p.district;  // ✅
    if (p.thana        !== undefined) fields[16] = p.thana;     // ✅
...
// Orders sheet — items JSON parse করা হয়
